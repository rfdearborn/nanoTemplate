...
class NeighborEncoderLayer(nn.Module):
    ...
    def __init__(self, config, use_cross_attention=True):
        ...
        self.mlp_fc = nn.Linear(config.n_embd, 4 * config.n_embd, bias=config.bias)
        self.gelu = nn.GELU()
        self.mlp_proj = nn.Linear(4 * config.n_embd, config.n_embd, bias=config.bias)
    ...
    def forward(self, x, h=None):
        ...
        # MLP
        residual = x
        x = self.mlp_proj(self.gelu(self.mlp_fc(self.ln_mlp(x))))
        x = residual + x
...
class NeighborEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.wte = None # Share w/ GPT after init
    ...
    def forward(self, neighbor_tokens, hidden_states):
        ...
        x = self.wte(neighbor_tokens) + self.wpe(pos)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, hidden_states, neighbor_embeddings):
        ...
        # Norms
        hidden_states = self.ln_hs(hidden_states)
        neighbor_embeddings = self.ln_ne(neighbor_embeddings)

        # Shift hidden states to maintain causality as in paper
        shift_amount = self.chunk_size - 1
        hidden_states_shifted = hidden_states[:, shift_amount:, :]
        hidden_states_shifted = F.pad(
            hidden_states_shifted,
            (0, 0, 0, shift_amount)
        )

        # Chunk hidden states
        hidden_states_shifted_chunks = hidden_states_shifted.view(B, n_chunks, self.chunk_size, C)

        # Project to q, k, v
        q = self.cross_attn_q(hidden_states_shifted_chunks)
        k, v = self.cross_attn_kv(neighbor_embeddings).split(C, dim=4)

        # Reshape for attention
        q = q.view(B * n_chunks, self.chunk_size, self.n_head, C // self.n_head).transpose(1, 2)
        k = k.view(B * n_chunks, k_neighbors * neighbor_size, self.n_head, C // self.n_head).transpose(1, 2)
        v = v.view(B * n_chunks, k_neighbors * neighbor_size, self.n_head, C // self.n_head).transpose(1, 2)
        ...
        # Shape: (B * n_chunks, chunk_size, C)
        
        # Project and reshape back
        x = x.transpose(1, 2).contiguous().view(B * n_chunks, self.chunk_size, C)
        x = self.cross_proj(x)
        x = x.view(B, T, C)
        
        # Restore padding and apply final MLP
        x = torch.cat([
            hidden_states[:, :shift_amount, :], # identity for dropped tokens, as in paper
            x[:, :-shift_amount, :]
        ], dim=1)
        x = self.mlp_proj(self.gelu(self.mlp_fc(self.ln_mlp(x))))
        ...
        for layer_index, block in enumerate(self.transformer['h']):
            ...
            if self.retrieval_enabled:
                ...
                if layer_index in retrieval_layers:
                    x = self.cross_attention_modules[retrieval_layer_index](x, neighbor_embeddings)
